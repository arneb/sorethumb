...
class ConvertFilter(Filter):
    """ Change mode of the Image to RGB """

    def __init__(self, mode):
        """
        :param mode: The new mode, e.g. 'RGB'

        """
        Filter.__init__(self, mode)
        self.new_mode = mode

    def __call__(self, img):
        img = img.convert(self.new_mode)
        return img


class ApplyExifRotation(Filter):
    """ Rotate image according to EXIF Ortientation """
    ...
    def __call__(self, img):
        ...
        if exif:
            ...
            # EXIF ORIENTATION specifies the following states:
            transformations = {
                # 1 = Horizontal (normal)
                1: lambda img: img,
                # 2 = Mirror horizontal
                ...
                5: lambda img: img.transpose(Image.FLIP_TOP_BOTTOM).transpose(Image.ROTATE_270),
                # 6 = Rotate 90 CW
            ...
            try:
                img = transformations.get(orientation)(img)
            except TypeError:
                pass
        return img
